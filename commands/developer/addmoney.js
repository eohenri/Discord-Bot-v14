...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!userdb) {
      const createDocUser = new User({ _id: user.id })
    ...
    await User.findByIdAndUpdate({ _id: user.id }, { $inc: { "economia.ametistas": valor, } });
        ...
        message.reply({ content: `As gemas foram adicionadas para ${user.username} no valor de ${valor} com sucesso!`})

        const emb = new EmbedBuilder()
          .setTitle("⚡ Adicionar moedas")
          .setDescription(`Você deu **${Number(valor).toLocaleString()} moedas** para ${user.username} com sucesso!`)
          ...
          const canal = await client.channels.fetch('1219861719145512960');
          canal.send({ content: `${user}`, embeds: [emb] });
